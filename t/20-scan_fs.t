...
my $old_fs = AnyEvent::Filesys::Watch->_scan_fs( $dir, "$dir/one" );
is( keys %$old_fs, 6, '_scan_fs: got all of them' );

create_test_files(qw(2 one/2 two/2));
my $new_fs = AnyEvent::Filesys::Watch->_scan_fs( [$dir] );
is( keys %$new_fs, 9, '_scan_fs: got all of them' );

my @events = AnyEvent::Filesys::Watch->_diff_fs( $old_fs, $new_fs );
is( @events, 3, '_diff_fs: got create events' ) or diag ddx @events;
is( $_->type, 'created', '... correct type' ) for @events;

$old_fs = $new_fs;
create_test_files(qw(2 one/2 two/2));
$new_fs = AnyEvent::Filesys::Watch->_scan_fs($dir);
@events = AnyEvent::Filesys::Watch->_diff_fs( $old_fs, $new_fs );
