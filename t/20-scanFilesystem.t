...
my $old_fs = AnyEvent::Filesys::Watch->_scanFilesystem($dir, "$dir/one");
...
my @events = AnyEvent::Filesys::Watch->_diffFilesystem( $old_fs, $new_fs );
is(@events, 3, '_diffFilesystem: got create events') or diag ddx @events;
is($_->type, 'created', '... correct type') for @events;

$old_fs = $new_fs;
create_test_files(qw(2 one/2 two/2));
$new_fs = AnyEvent::Filesys::Watch->_scanFilesystem($dir);
@events = AnyEvent::Filesys::Watch->_diffFilesystem($old_fs, $new_fs);
