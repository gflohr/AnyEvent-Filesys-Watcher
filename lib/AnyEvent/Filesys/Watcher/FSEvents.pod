...
Specifies the latency.  The latency is the time that the fsevents daemon waits
for possibly merging multiple events.  A higher value therefore potentially
reduces the amount of events created.
